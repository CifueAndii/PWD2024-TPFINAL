...
class ControlCarrito{
    ...
    public function altaCompraItem($param)
    {
        ...
        $param["producto"] = $objProducto->buscar(["id" => $param["idproducto"]]);
        $param["carrito"] = $objControl->retornarCarrito($param);

        $rta = $objControl->agregarCarrito($param);

        if (!$rta["bandera"]) {
            $result["errorMsg"] .= "No contamos con stock para esa cantidad. ";
            $rta["respuesta"] = false;
        }
        if (!$rta["respuesta"]) {
            $result["errorMsg"] .= "No se pudo concretar la accion.";
        }

        $result["respuesta"] = $rta["respuesta"];

        return $result;
    }

    public function listarCompraItem($param)
    {
        $objControl = new AbmCompra();
    ...
    public function modificarCompraItem($param)
    {
        ...
        if (isset($data['idcompraitem'])) {
            ...
            $arreglo = $objC->buscarItems($param);

            if ($arreglo[0]->getObjProducto()->getCantStock() >= $param["cantidad"]) {
                $respuesta = $objC->modificarItem($param);
            } else {
